...
const Login = () => {
  ...
  const onSubmit = async (data: FormData) => {
    login(data.email, data.password)
      .json((res: { accessToken: string; refreshToken: string }) => {
        storeToken(res.accessToken, "access");
        storeToken(res.refreshToken, "refresh");
        router.push("/dashboard");
      })
      .catch((err) => {
        setError("root", { type: "mamual", message: err.json.detail });
      });
  };
